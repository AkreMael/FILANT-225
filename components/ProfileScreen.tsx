...
const ProfileScreen: React.FC<ProfileScreenProps> = ({ user, onClose, onLogout, onReset, isClientModeActive, onToggleClientMode, setActiveTab, onShowPopup }) => {
  ...
  const handleModeToggleClick = () => {
    const initialRole = localStorage.getItem('filant_user_role');
    const savedProRole = localStorage.getItem('filant_selected_pro_role');
    const isInitialPro = initialRole && initialRole !== 'Client' && initialRole !== 'Admin 225';
    if (isClientModeActive) {
      if (isInitialPro || savedProRole) {
        onToggleClientMode(false);
      } else {
        setView('mode-selection');
      }
    } else {
      onToggleClientMode(true);
    }
  };

  const handleSelectProMode = (mode: string) => {
    onShowPopup(`Confirmer le mode ${mode} ?`, 'confirm', (close) => {
      onToggleClientMode(false, mode);
      close();
    });
  };
  ...
  const renderModeSelection = () => (
    <div className="flex flex-col h-full bg-white animate-in slide-in-from-right duration-300">
        <header className="p-4 flex items-center bg-white shadow-sm border-b border-gray-100">
            ...
            <h1 className="flex-1 text-center font-black uppercase text-base tracking-tight mr-10">Activation des services</h1>
        </header>
        <div className="p-8 flex-1 flex flex-col items-center justify-center text-center">
            <div className="w-20 h-20 bg-orange-100 rounded-full flex items-center justify-center mb-6 shadow-inner">
                <UsersIcon className="w-10 h-10 text-orange-500" />
            </div>
            <h2 className="text-2xl font-black text-slate-900 leading-tight mb-8">
                Souhaitez-vous avoir accès à nos services ?
            </h2>
            <div className="w-full space-y-4 max-w-xs">
                {[{ id: 'Travailleur', label: 'Mode Travailleur', color: 'bg-[#FF4500]' }, { id: 'Propriété', label: 'Mode Propriétaire d’équipement', color: 'bg-[#2E8B57]' }, { id: 'Agence', label: 'Mode Agence immobilière', color: 'bg-[#4682B4]' }].map(m => (
                    <button key={m.id} onClick={() => handleSelectProMode(m.id)} className={`w-full py-4 px-6 rounded-2xl text-white font-black uppercase text-xs tracking-widest shadow-lg transform active:scale-95 transition-all ${m.color}`}>
                        {m.label}
                    </button>
                ))}
            </div>
        ...
        <div className="p-6 text-center">
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-relaxed">Ce choix déterminera l'interface professionnelle qui vous sera proposée. <br/> Vous pourrez toujours revenir en mode client.</p>
        </div>
    </div>
  );
  ...
  const renderMainView = () => (
    <div className="flex flex-col h-full bg-[#F3F3F3]">
        ...
        <div className="flex-1 space-y-6 pb-32">
            ...
            <div className="bg-white rounded-3xl overflow-hidden mx-4 shadow-sm border border-gray-100">
                 <ProfileRow icon={<FavorisIcon className="w-10 h-10 text-indigo-600" />} title="Historique" subtitle="Favoris & Demandes" onClick={() => { setFavorites(databaseService.getFavorites(user.phone)); setView('favorites'); }} />
            </div>
            <div className="mx-4">
                <ProfileRow isDark icon={<AdminIcon className="w-10 h-10 text-yellow-400" />} title="Compte professionnel" subtitle="Accès Administrateur Admis" onClick={() => { if (user.phone === ADMIN_PHONE) { setActiveTab(Tab.AdminDashboard); handleClose(); } else { setActiveTab(Tab.AdminLogin); handleClose(); } }} rightElement={<ChevronRight className="text-red-500" />} />
            </div>
            <div className="bg-white rounded-3xl overflow-hidden mx-4 shadow-sm border border-gray-100">
                <div className="h-px bg-gray-50 mx-4"></div>
                {isClientModeActive && (
            ...
            <main className="flex-1 overflow-y-auto scrollbar-hide">
                ...
                {view === 'favorites' && (
                    <div className="bg-[#F3F3F3] h-full flex flex-col">
                        ...
                        <FavoritesListView favorites={favorites} onSelect={(fav) => { setSelectedFavorite(fav); setView('favorite-detail'); }} onClearAll={() => onShowPopup("Voulez-vous vider tout votre historique ?", "confirm", (close) => { databaseService.clearFavorites(user.phone); setFavorites([]); close(); })} onDeleteOne={(id) => { databaseService.removeFavorite(user.phone, id); setFavorites(prev => prev.filter(f => f.id !== id)); }} />
                    </div>
                )}
                {view === 'contacts' && (
                    <div className="bg-[#F3F3F3] h-full flex flex-col">
