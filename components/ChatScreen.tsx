...
const ChatScreen: React.FC<ChatScreenProps> = ({ currentUser, targetUser, isAdmin, onBack }) => {
  ...
  const handleApprovePublication = async (requestData: any) => {
    ...
    await handleSendMessage(confirmMessage, 'admin');
  };
  ...
  useEffect(() => {
    // Auto-reply for RECRUIT_REQUEST
    if (!isAdmin && displayMessages.length > 0) {
      const lastMessage = displayMessages[displayMessages.length - 1];
      if (lastMessage.sender === 'user' && lastMessage.text.startsWith('RECRUIT_REQUEST:')) {
        const alreadyReplied = displayMessages.some(msg => 
          msg.sender === 'admin' && 
          msg.text.startsWith('RECRUIT_CONFIRM:') && 
          msg.timestamp > lastMessage.timestamp
        );

        if (!alreadyReplied) {
          const data = lastMessage.text.replace('RECRUIT_REQUEST:', '');
          setTimeout(() => {
            handleSendMessage(`RECRUIT_CONFIRM:${data}`, 'admin');
          }, 1000);
        }
      }
    }
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-hide bg-slate-50/50 overscroll-contain">
        {isLoading ? (
          ...
          displayMessages.map((msg, idx) => {
            ...
            } else if (msg.text.startsWith('RECRUIT_CONFIRM:')) {
              try {
                const data = JSON.parse(msg.text.replace('RECRUIT_CONFIRM:', ''));
                content = (
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-black text-[10px] uppercase tracking-widest text-blue-600">Confirmation de recrutement</span>
                    </div>
                    <p className="text-xs font-bold text-slate-600 leading-relaxed">
                      Votre demande de recrutement pour <span className="text-slate-900">"{data.service}"</span> a été reçue. Veuillez procéder au paiement pour finaliser la mise en relation.
                    </p>
                    {!isAdmin && (
                      <div className="space-y-2">
                        <button 
                        ...
                        >
                          ...
                          Payer pour recruter (500F)
                        </button>
                        <button 
                          onClick={() => handleWhatsAppContact(data)}
                          className="w-full py-4 bg-green-500 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-xl shadow-green-200 active:scale-95 transition-all flex items-center justify-center gap-2"
                        >
                ...
                console.error("Error parsing RECRUIT_CONFIRM", e);
              }
